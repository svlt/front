	...
	crypto: {
		...
		pgp: {
			...
			decrypt: function(message, privateKey, callback) {
				...
				privateKey = openpgp.key.readArmored(privateKey);
				return openpgp.decryptMessage(privateKey.keys[0], message).then(function(data) {
					callback(data);
				});
			},
			decryptAndVerify: function(message, privateKey, publicKey, callback) {
				...
				publicKey = openpgp.key.readArmored(publicKey);
				privateKey = openpgp.key.readArmored(privateKey);
				return openpgp.decryptAndVerifyMessage(privateKey.keys[0], publicKey.keys, message).then(function(data) {
